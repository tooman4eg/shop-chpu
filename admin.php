<?php
  ...
  switch ($eaction)
  {
      case 'cat':
          ...
          $q = db_query("select categoryID, name, products_count, products_count_admin, parent, picture, subcount, Activation_status FROM ".CATEGORIES_TABLE." ORDER BY sort_order, name");
          $fc = array(); //parents
          $mc = array(); //parents
		  ...
		  $Activation_status = isset($_POST["Activation_status"]) ? 1 : 0;
...

              {
                  $q = db_query("insert into ".CATEGORIES_TABLE." (name, parent, products_count, description, picture, ".
                       ...
                       " meta_description, meta_keywords, title, Activation_status ) "." VALUES ('".xToText(trim($_POST["name"]))."', ".
                       (int)$_POST["parent"].",0,'".xEscSQL($_POST["desc"])."','',0, ".(int)$_POST["sort_order"].", ".$allow_products_comparison.", ".$allow_products_search.", ".
                       $show_subcategories_products.", '".xToText(trim($_POST["meta_d"]))."', '".xToText(trim($_POST["meta_k"]))."', '".xToText(trim($_POST["title"]))."', '".$Activation_status."');");
...

                  {
                      ...
                      if (category_Moves_To_Its_SubDirectories($_POST["must_delete"], $_POST["parent"]))
                      {
                          ...
                          db_query("update ".CATEGORIES_TABLE." SET name='".xToText(trim($_POST["name"]))."', description='".xEscSQL($_POST["desc"])."', parent=".(int)$_POST["parent"].
                              ...
                              "', title='".xToText(trim($_POST["title"]))."', Activation_status ='".$Activation_status."'   WHERE categoryID=".(int)$_POST["must_delete"]);
                      }
...
<tr><td align="left">
<input type="checkbox" name="Activation_status" id="Activation_status" value='1'
<?php
...
></td><td align="left" width="100%"><label for="allow_products_comparison"> &nbsp;<?php
              echo "Показывать ли категорию в списке?";
?></label></td></tr>
