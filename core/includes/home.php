...
#####################################
        ...
        $cifra = 12; //количество последних товаров дл€ выбора
        $result = array();

		//shopcms  -говно
		//$q = db_query("select s.productID, s.name, s.Price, s.enabled, t.filename FROM ".PRODUCTS_TABLE." AS s LEFT JOIN ".PRODUCT_PICTURES."
		
        $q = db_query("select s.productID, s.name, s.Price, s.enabled, t.filename, in stock FROM ".PRODUCTS_TABLE." AS s LEFT JOIN ".PRODUCT_PICTURES."
        ...
        $q = db_query("select * FROM ".PRODUCTS_TABLE." AS s LEFT JOIN ".PRODUCT_PICTURES."
        AS t on (s.default_picture=t.photoID AND s.productID=t.productID) WHERE s.categoryID!=1 AND s.enabled=1 ORDER BY s.date_added DESC LIMIT 0,".$cifra);
        ...
        $cifra = 8; //количество последних товаров дл€ выбора
        $result = array();
...
/*ƒобавили выборку из банеров*/
 $result = array();
 $q = mysql_query("SELECT * FROM ".BANNERS_TABLE." WHERE active=1 ORDER BY id");
 while ($row = mysql_fetch_array($q)) {
	$row['img'] = "data/banners/".$row['img'];
	$row['name'] = $row['name'];
	if ($row['url'] == '') {$row['url'] = "#";}
	else {
