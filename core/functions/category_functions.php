...
function fillTheCList($parent,$level) //completely expand category tree
{
		...
		$q = db_query("select * FROM ". CATEGORIES_TABLE." WHERE parent=".(int)$parent." ORDER BY sort_order, name");
        $a = array(); //parents
        while ($row = db_fetch_row($q))
        {
                $row["level"] = $level;
...
function getcontentcatresc( $catID )
{
		...
		$q = db_query( "select * from ".CATEGORIES_TABLE." where parent=".(int)$catID." order by sort_order, name " );
        $res = array();
...
function catGetCategoryCompactCList( $selectedCategoryID )
{
        ...
        $q = db_query( "select categoryID, parent, name, products_count , Activation_status from ".CATEGORIES_TABLE.		
                                " where parent=1 ".
...
{
        $q = db_query("select categoryID, name, products_count, Activation_status FROM ".
...
function catGetCategoryById($categoryID)
{
        $q = db_query("select categoryID, name, parent, products_count, description, picture, ".
                ...
                " show_subcategories_products, meta_description, meta_keywords, title, Activation_status ".
