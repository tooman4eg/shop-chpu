...
function _moveSessionCartContentToOrderedCart( $orderID )
{
        ...
        foreach( $_SESSION["gids"] as $productID )
        {
                ...
                foreach( $_SESSION["configurations"][$i] as $vars )
                {
					...
					// делаем запись в demo_item_options (доп.параметры для этого товара)											
					// db_query("INSERT INTO `demo_item_options` (itemID, optionID, orderID, variantID, price_surplus) 
													// VALUES ('".$last_itemID."',
															// '".$option["optionID"]."',
															// '".$orderID."',
															// '".$option["variantID"]."',
